...
class Obi:
    ...
    def run(self, input_fasta_file, include_analysis=True, min_identity=None, max_evalue=None, min_coverage=None,
        ...
        fasta_file = self._blast.run(input_fasta_file, self._results_dir, min_identity=min_identity,
        ...
        if include_analysis:
            return PositiveSelectionAnalyzer.for_mode(mode).analyse(self._results_dir, alignment_preparation_result,
                                                                    email=self._email)
